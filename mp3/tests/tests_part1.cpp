...
TEST_CASE("List::reverse", "[weight=1][part=1]") {
  ...
  PNG expected;
  expected.readFromFile("tests/ExpectedImages/expected-reserve.png");

  List<HSLAPixel> list = imageToList(in);
  list.reverse();
  PNG out = listToImage(list, in.width(), in.height());
  out.writeToFile("tests/Results/actual-reserve.png");
...
TEST_CASE("List::reverseNth #1", "[weight=1][part=1]") {
  ...
  in.readFromFile("tests/alma.png");
  PNG expected;
  expected.readFromFile("tests/ExpectedImages/expected-reserveN_1.png");

  List<HSLAPixel> list = imageToList(in);
  list.reverseNth(in.height() * 20);

  PNG out = listToImage(list, in.width(), in.height());
  out.writeToFile("tests/Results/actual-reserveN_1.png");
...
TEST_CASE("List::reverseNth #2", "[weight=1][part=1]") {
  ...
  in.readFromFile("tests/alma.png");
  PNG expected;
  expected.readFromFile("tests/ExpectedImages/expected-reserveN_2.png");

  List<HSLAPixel> list = imageToList(in);
  list.reverseNth(in.height() * 61);

  PNG out = listToImage(list, in.width(), in.height());
  out.writeToFile("tests/Results/actual-reserveN_2.png");
...
TEST_CASE("List::waterfall", "[weight=1][part=1]") {
  ...
  in.readFromFile("tests/alma.png");
  PNG expected;
  expected.readFromFile("tests/ExpectedImages/expected-waterfall.png");

  List<HSLAPixel> list = imageToList(in);
  list.waterfall();
  PNG out = listToImage(list, in.width(), in.height());
  out.writeToFile("tests/Results/actual-waterfall.png");
