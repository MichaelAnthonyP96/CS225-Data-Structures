...
void List<T>::reverse(ListNode *&startPoint, ListNode *&endPoint) {
  ...
  // must save the head and the tail pointers so that we can reassign them at
  // the end
  ListNode *originalFrontPrev = startPoint->prev;
  ListNode *originalBackNext = endPoint->next;

  // iterates through the list and switches their next and prev pointers, moving
...
template <class T> void List<T>::waterfall() {
  ...
  while (curr->next->next != NULL && temp1->next != NULL) {
    ...
    tail_->next = curr;
    // move temp1 to the next node after the jump, which curr's next still
    // points to
    temp1 = curr->next;
...
typename List<T>::ListNode *List<T>::split(ListNode *start, int splitPoint) {
  ...
  if (start == NULL || splitPoint <= 0)
    ...
    // now I am at the start of the new list, need to disconnect from the
    // original list
...
typename List<T>::ListNode *List<T>::mergesort(ListNode *start,
  ...
  // base case of when the chain-length is 1 or 0
  if (chainLength <= 1) {
    start->next = NULL;
    start->prev = NULL;
    return start;
