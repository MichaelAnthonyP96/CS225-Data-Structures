...
template <class T> int List<T>::size() const { return length_; }

template <class T> List<T>::List(List<T> const &other) { copy(other); }

template <class T> List<T> &List<T>::operator=(List<T> const &rhs) {
  if (this != &rhs) {
    clear();
    copy(rhs);
  }
...
template <class T> void List<T>::copy(List<T> const &other) {
...
template <class T> void List<T>::print(ostream &os) const {
  os << "<";
  ListNode *curr = head_;
  while (curr != NULL) {
    os << " " << curr->data;
