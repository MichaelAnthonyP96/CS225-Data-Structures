...
LogfileParser::LogfileParser(const string &fname) : whenVisitedTable(256) {
  ...
  while (infile.good()) {
     ...
     * vector as well.
     */
    // extract the new Key from logline, not sure why you ignore the last two
    // characters but the given code does it so I followed what they did
    string newKey = ll.customer.substr(1, ll.customer.size() - 3);
    string uniqueURL = ll.url;
    newKey.append(uniqueURL);
    // if the key exists and was visited in the past then update its last
    // visited date
    if (whenVisitedTable.keyExists(newKey)) {
      if (whenVisitedTable[newKey] < ll.date)
        whenVisitedTable[newKey] = ll.date;
    }
    // if it hasn't been visited then insert it into the visited Table
    else {
      whenVisitedTable.insert(newKey, ll.date);
    }
    // if the URL does not exist in the page Visited table then insert it into
    // vector and mark it as true in the page Visited table
...
bool LogfileParser::hasVisited(const string &customer,
   ...
   */
  // extracts the key minus the last two characters, given code does it but Im
  // not sure why
