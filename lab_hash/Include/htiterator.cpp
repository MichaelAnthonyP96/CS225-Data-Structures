...
template <class K, class V> HashTable<K, V>::iterator::iterator() : impl(NULL) {
  /* nothing */
}
...
template <class K, class V>
const typename HashTable<K, V>::iterator &
...
template <class K, class V> HashTable<K, V>::iterator::~iterator() { clear(); }
...
template <class K, class V> void HashTable<K, V>::iterator::clear() {
  delete impl;
  impl = NULL;
}
