...
template <class K, class V> LPHashTable<K, V>::LPHashTable(size_t tsize) {
  if (tsize <= 0)
    tsize = 17;
...
template <class K, class V> LPHashTable<K, V>::~LPHashTable() {
  for (size_t i = 0; i < size; i++)
    delete table[i];
  delete[] table;
  delete[] should_probe;
...
template <class K, class V> void LPHashTable<K, V>::remove(K const &key) {
...
template <class K, class V> V LPHashTable<K, V>::find(K const &key) const {
  int idx = findIndex(key);
  if (idx != -1)
...
template <class K, class V> V &LPHashTable<K, V>::operator[](K const &key) {
...
template <class K, class V> void LPHashTable<K, V>::clear() {
  for (size_t i = 0; i < size; i++)
    delete table[i];
  delete[] table;
  delete[] should_probe;
...
template <class K, class V> void LPHashTable<K, V>::resizeTable() {
