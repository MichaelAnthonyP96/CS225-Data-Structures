...
TEST_CASE("testRemoveEASY", "[weight=6]") {
  TextFile infile("texts/textEasy.txt");
...
TEST_CASE("testRemoveHard", "[weight=10]") {
  TextFile infile("texts/textHard.txt");
  SCHashTable<string, int> hashTable(32);
  while (infile.good()) {
    string word = infile.getNextWord();
...
TEST_CASE("testResizeOnce", "[weight=7]") {
  TextFile infile("texts/textLong.txt");
...
template <template <class K, class V> class HT> void testAnagrams() {
  vector<string> strings;
...
template <template <class K, class V> class HT> void testWordCount() {
  WordFreq<HT> wf("texts/wordCount.txt");
...
TEST_CASE("testLogfileParse", "[weight=18]") {
  LogfileParser lfp("texts/log.txt");
