...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
    std::unordered_map<char,int> dict1(256);
    std::unordered_map<char,int> dict2(256);

    for (unsigned long i=0; i < word.size(); i++ ){ //now make a dict
        dict1[word[i]] += 1;
    }

    for (unsigned long i=0; i < test.size(); i++ ){ //now make a dict
        dict2[test[i]] += 1;
    }

    for (auto i = dict1.begin(); i != dict1.end(); i++){//compare
        if(dict1[(*i).first]!=dict2[(*i).first] ){
            return false;
        }
    }
