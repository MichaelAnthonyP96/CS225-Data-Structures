...
for(unsigned int x = 0; x < image.width(); x++){
    for(unsigned int y = 0; y < image.height(); y++){
        ...
        HSLAPixel & pixel = image.getPixel(x,y);
        //Get the distance from the spotlight
        double distance = sqrt((centerX - x)*(centerX - x) + (centerY - y)*(centerY - y));
        if(distance > 160){
          pixel.l = pixel.l * 0.2;
        }
        else{
          //adjust the luminance based on the distance from the spotlight
...
for(unsigned int x = 0; x < firstImage.width(); x++){
  for(unsigned int y = 0; y < firstImage.height(); y++){
    //Get a reference to the pixel at (x,y) of the first image:
    HSLAPixel & pixel1 = firstImage.getPixel(x,y);
    //Get a reference to the pixel at (x,y) of the second image:
    HSLAPixel & pixel2 = secondImage.getPixel(x,y);
    //If the second image has a luminance of 1, increase the first image luminance
    if(pixel2.l ==  1){
