...
namespace std {
 ...
 class ourStack : public stack<T, Container> {
        public: 
            ...
            explicit ourStack(const Container& c) : stack<T, Container>(c) { _ctor_count ++; };
            explicit ourStack(Container&& c) : stack<T, Container>(c) { _ctor_count ++; };

            template <class Alloc> explicit ourStack(const Alloc& a) : stack<T, Container>(a) { _ctor_count ++; };
            template <class Alloc> ourStack(const Container& c, const Alloc& a) : stack<T, Container>(c, a) { _ctor_count ++; };
            template <class Alloc> ourStack(Container&& c, const Alloc& a) : stack<T, Container>(c, a) { _ctor_count ++; };
            template <class Alloc> ourStack(const ourStack& c, const Alloc& a) : stack<T, Container>(c, a) { _ctor_count ++; };
            template <class Alloc> ourStack(ourStack&& c, const Alloc& a) : stack<T, Container>(c, a) { _ctor_count ++; };
