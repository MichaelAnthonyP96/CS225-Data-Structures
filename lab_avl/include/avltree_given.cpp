...
template <class K, class V> AVLTree<K, V>::~AVLTree() { clear(root); }

template <class K, class V> vector<K> AVLTree<K, V>::getInorderTraversal() {
...
template <class K, class V> vector<string> AVLTree<K, V>::getFunctionOrder() {
...
template <class K, class V> void AVLTree<K, V>::clear(Node *subRoot) {
  if (subRoot == NULL)
    return;
...
template <class K, class V> void AVLTree<K, V>::setOutput(ostream &newOut) {
  _out = &newOut;
}
