...
public:
  ...
  template <typename T> static void output_red(T c) {
    std::cout << "\033[31m" << c << "\033[39m";
  }

  template <typename T> static void output_green(T c) {
    std::cout << "\033[32m" << c << "\033[39m";
  }

  template <typename T> static void output_notfound(T c) {
  ...
  static void output_bold_digits(std::string str, std::ostream &out = std::cout,
                                 enable_t enable = COUT) {
    if (!is_enabled(out, enable)) {
      out << str;
      return;
