...
template <class DerivedDescriptor> class GenericNodeDescriptor {
public:
...
template <class NodeDescriptor> int getPrintHeight(NodeDescriptor croot);
...
void printSubtree(NodeDescriptor croot, std::vector<std::string> &output,
  ...
  for (size_t i = 0;
       i < nodeStr.length() && left + curr_width / 2 + i < output[top].length();
       i++)
