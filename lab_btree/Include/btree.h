...
size_t insertion_idx(const std::vector<T> &elements, const C &val) {
  ...
  // gets the number of elements in the standard vector and type casts it to the
  // needed type
  size_t numElements = size_t(elements.size());
  // if the vector is empty then return index 0
  if (numElements == 0)
    return numElements;
  // if val is larger than the last value in the vector, return the size of the
  // vector
  if (elements[numElements - 1] < val)
    return numElements;
