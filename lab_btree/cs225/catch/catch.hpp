...
template <typename T> struct is_string_array : std::false_type {};

template <std::size_t N> struct is_string_array<char[N]> : std::true_type {};
...
template <typename T> struct BoundRef : BoundValueRefBase {
  T &m_ref;
...
template <typename T> struct BoundRef<std::vector<T>> : BoundValueRefBase {
  std::vector<T> &m_ref;
