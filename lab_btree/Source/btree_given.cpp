 ...
 */
template <class K, class V> BTree<K, V>::BTree() {
  root = nullptr;
  order = 64;
}
 ...
 */
template <class K, class V> BTree<K, V>::BTree(unsigned int order) {
  root = nullptr;
 ...
 */
template <class K, class V> BTree<K, V>::BTree(const BTree &other) {
  clear(root);
 ...
 */
template <class K, class V> void BTree<K, V>::clear(BTreeNode *subroot) {
  if (!subroot->is_leaf) {
    for (auto child : subroot->children) {
      clear(child);
    }
 ...
 * Destroys a BTree.
 */
template <class K, class V> BTree<K, V>::~BTree() { clear(); }
 ...
 */
template <class K, class V> void BTree<K, V>::clear() {
  if (root != nullptr) {
    clear(root);
    root = nullptr;
  }
