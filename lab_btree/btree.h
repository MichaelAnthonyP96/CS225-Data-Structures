...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    //gets the number of elements in the standard vector and type casts it to the needed type
    size_t numElements = size_t(elements.size());
    //if the vector is empty then return index 0
    if(numElements == 0) return numElements;
    //if val is larger than the last value in the vector, return the size of the vector
    if(elements[numElements-1] < val) return numElements;
    //if the val occurs in the elements then return index of val in the vector
    if(elements[numElements-1] == val) return numElements-1;
    //if val is smaller than the first element then return 0
    if(elements[0] > val) return 0;
    //now we know the index is somewhere in the interior if the vector
    else{
        size_t low = size_t(0);
        size_t high = size_t(numElements-1);
        while(low < high){
            //perform binary search
            size_t mid = ((low + high) / 2);
            // found a place to insert the Pair
            if((elements[mid] < val) && elements[mid+1] > val) return mid+1;
            // found a repeated Key
            if(elements[mid] == val) return mid;
            // shift the interval to the left
            if(elements[mid] > val){
                high = mid;
                continue;
            }
        ...
        //low and high are equal, return low
        return low;
    }
}
