...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
    //if first_larger_idx does not exceed the length of the vector and the subroot is not empty
    if((first_larger_idx < subroot->elements.size()) && !(subroot->elements.empty())){
    	//if the element at the first_larger_idx of the current subroot is equal to the key,
    	//return its value. Otherwise search its children
    	if(subroot->elements[first_larger_idx].key == key){
    		return subroot->elements[first_larger_idx].value;
    	}
    }
    /*If the subroot is a leaf and we didn't find the key in it, then we have failed to find it
     * anywhere in the tree and return the default V.
     */
    if(subroot->is_leaf)
    	return V();
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    if(!new_left->is_leaf){
    	//assign children to the new right element, pointers to the old right hand children
    	new_right->children.assign(mid_child_itr, new_left->children.end());
    	//assign children to the new left element, pointers to the old left hand children
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    //base cases
    //checking to see if the current subroot is valid and the key doesnt already exist
    if((first_larger_idx < subroot->elements.size()) && !subroot->elements.empty()){
    ...
    if(subroot->is_leaf){
    	//if the subroot is a leaf then insert the pair into its elements at the correct index
    ...
    //subroot is not a leaf and the key doesn't exist in subroot
    	...
    	insert(Node, pair);
    	//once we have inserted, check to see if child became too big and needs to be split
