...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies) {
  ...
  while (singleQueue.size() + mergeQueue.size() > 1) {
    ...
    TreeNode *internalNode = new TreeNode(smallest->freq.getFrequency() +
...
void HuffmanTree::decode(stringstream &ss, BinaryFileReader &bfile) {
  ...
  while (bfile.hasBits()) {
     ...
     *check if we are a leaf node print this character to the stringstream if a
     *leaf, reset current to top of tree */
