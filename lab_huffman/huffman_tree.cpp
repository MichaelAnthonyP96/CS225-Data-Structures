                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    //if both queue's are empty then return null
    if(singleQueue.empty() && mergeQueue.empty()) return NULL;
    //if singleQueue is empty and mergeQueue is not then use mergeQueue's front
    if(singleQueue.empty()){
        smallestNode = mergeQueue.front();
        mergeQueue.pop();
        return smallestNode;
    }
    //if mergeQueue is empty and singleQueue is not then use singleQueue's front
    if(mergeQueue.empty()){
        smallestNode = singleQueue.front();
        singleQueue.pop();
        return smallestNode;
    }
    //if singleQueue's front is greater than use the mergeQueue's front
    if(singleQueue.front()->freq.getFrequency() > mergeQueue.front()->freq.getFrequency()){
        smallestNode = mergeQueue.front();
        mergeQueue.pop();
        return smallestNode;
    }
    //implements tiebreaking of using the singleQueue's front
    else{
        smallestNode = singleQueue.front();
        singleQueue.pop();
        return smallestNode;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     * the back of the mergeQueue.*/
     while(singleQueue.size() + mergeQueue.size() > 1){
       ...
       TreeNode* internalNode = new TreeNode(smallest->freq.getFrequency() + secondSmallest->freq.getFrequency());
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    * given */
    ...
    while (bfile.hasBits()) {
        ...
        //move temp right if the bit is 1
        if(bit) current = current->right;
        //move temp left if the bit is 0
        ...
        *check if we are a leaf node print this character to the stringstream if a leaf, reset current to top of tree */
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     /* creates a new heap TreeNode and assigns its left and right subtree*/
