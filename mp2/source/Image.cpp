...
Image::Image(const Image &other)
    : PNG(other.getWidth(), other.getHeight()) { // copy constructor
...
void Image::scale(double factor) {
  if (factor <= 0) {
    ...
    std::cout << "Cannot scale an image by a factor of 0 or less. "
              << std::endl;
  } else {
    ...
    // rescale the image before or after the depending on if shrinking or
    // expanding
    if (factor > 1.0) {
      ...
      for (int x = newWidth - 1; (x > (-1)); x--) {
        for (int y = newHeight - 1; (y > (-1)); y--) {
          ...
          cs225::HSLAPixel *pixelo =
              &this->cs225::PNG::getPixel(int(x / factor), int(y / factor));
...
This function preserves the aspect ratio of the image, so the result will always
be an image of width w or of height h (not necessarily both). This function both
resizes the Image and scales the contents. Parameters w	Desired width of the
scaled Image h	Desired height of the scaled Image
*/
...
unsigned int Image::getWidth() const { return this->cs225::PNG::width(); }
// \brief returns Image height
unsigned int Image::getHeight() const { return this->cs225::PNG::height(); }

void Image::clean(double threshold) {
  for (int x = 0; x < this->width(); ++x) {
    for (int y = 0; y < this->height(); ++y) {
      // Get a pointer to the pixel at (x,y):
      HSLAPixel *pixel = &this->cs225::PNG::getPixel(x, y);
...
void Image::trim(int startW, int endW, int startH, int endH) {
  if (startW > this->width() || endW > this->width() ||
      startH > this->height() || endH > this->height()) {
    std::cout << "Invalid trim parameters!!!" << std::endl;
  }
  int newSize = ((endW - startW) + 1) * ((endH - startH) + 1);
  HSLAPixel *newData = new HSLAPixel[newSize];
  int index;
  for (int x = startW; x <= endW; ++x) {
    for (int y = startH; y <= endH; ++y) {
      // Copy the pixel at (x,y):
      index = x + (y * this->width());
      HSLAPixel pixel = imageData_[index];
    }
  }

  // update width and height
  this->setWidth((endW - startW) + 1);
  this->setHeight((endH - startH) + 1);
  // Clear old data and use the trimmed data
  delete[] imageData_;
  imageData_ = newData;
}