...
SquareMaze::~SquareMaze() { clear(); }

// destructor helper function
void SquareMaze::clear() {
  // iterate over the array of cell array pointers to over arrays and delete
  // them
  for (int i = 0; i < w; ++i) {
    delete[] maze[i];
  }
...
void SquareMaze::makeMaze(int width, int height) {
  ...
  while (count != (totalCells - 1)) {
    ...
    // rand is seeded correctly that we get different mazes each time we call
    // make maze
    xPos = std::rand() % w;
    yPos = std::rand() % h;
    ...
    if ((removeWall == false) && ((yPos + 1) < h)) {
      ...
      if ((sets.find(h * xPos + yPos)) != (sets.find(h * xPos + yPos + 1))) {
        count++;
        sets.setunion(sets.find(h * xPos + yPos),
                      sets.find(h * xPos + yPos + 1));
        maze[xPos][yPos].bottomWall = false;
      }
    }
    // check if its a valid xPos and removeWall is true
    else if ((removeWall == true) && ((xPos + 1) < w)) {
      // if the cell and the cell to its right it are not in the same set, union
      // them, remove wall, and increase count of visited cells
      if ((sets.find(h * (xPos + 1) + yPos)) != (sets.find(h * xPos + yPos))) {
        count++;
        sets.setunion(sets.find(h * (xPos + 1) + yPos),
                      sets.find(h * xPos + yPos));
...
bool SquareMaze::canTravel(int x, int y, int dir) const {
  ...
  // move right
  case 0: {
    // check if the cell's right wall is true, or if we are at the right end of
    // the maze
    if ((x >= w - 1) || (maze[x][y].rightWall == true))
      return false;
    else
      return true;
  }
  // move down
  case 1: {
    // check if the cell's bottom wall is true, or if we are at the bottom end
    // of the maze
    if ((y >= h - 1) || (maze[x][y].bottomWall == true))
      return false;
    else
      return true;
  }
  // move left
  case 2: {
    // check if the cell to the left's right wall is true, or if we are at the
    // left end of the maze
    if ((x == 0) || (maze[x - 1][y].rightWall == true))
      return false;
    else
      return true;
  }
  // move up
  case 3: {
    // check if the cell above's bottom wall is true, or if we are at the top of
    // the maze
    if ((y == 0) || (maze[x][y - 1].bottomWall == true))
      return false;
    else
      return true;
  }
 ...
 *(same as the encoding explained in canTravel). You only need to support
 *setting the bottom and right walls of every square in the grid. exists
 *true if setting the wall to exist, false otherwise
 */
 ...
 * Hint: this function should run in time linear in the number of cells in the
 *maze. Returns a vector of directions taken to solve the maze
 */
vector<int> SquareMaze::solveMaze() {
  ...
  // vector of negative ones to store the previous cell for all cells to begin
  // with
  vector<int> previous(w * h, -1);
  // vector of zero's to store the distance from the start for all cells to
  // begin with
  ...
  for (int k = 0; k < h; ++k) {
    for (int r = 0; r < w; ++r) {
      if (maze[k][r].rightWall == true) {
        ...
        for (int g = 0; g <= 10; ++g) {
          cs225::HSLAPixel &currentCellRight =
              mazePNG->getPixel((k + 1) * 10, (r * 10) + g);
      ...
      if (maze[k][r].bottomWall == true) {
        ...
        for (int g = 0; g <= 10; ++g) {
          cs225::HSLAPixel &currentCellBottom =
              mazePNG->getPixel((k * 10) + g, (r + 1) * 10);
 ...
 *drawMaze to show the solution vector and the exit. Start at pixel (5,5). Each
 *direction in the solution vector corresponds to a trail of 11 red pixels in
 *the given direction. If the first step is downward, color pixels (5,5) through
 *(5,15) red. (Red is 0,1,0.5,1 in HSLA). Then if the second step is right,
 *color pixels (5,15) through (15,15) red. Then if the third step is up, color
 *pixels (15,15) through (15,5) red. Continue in this manner until you get to
 *the end of the solution vector, so that your output looks analogous the above
 *picture. Make the exit by undoing the bottom wall of the destination square:
 *call the destination maze coordinates (x,y), and whiten the pixels with
 *coordinates (x*10+k, (y+1)*10) for k from 1 to 9. Returns a PNG of the solved
 *SquareMaze
 */
cs225::PNG *SquareMaze::drawMazeWithSolution() {
  ...
  for (int k = 1; k <= 9; ++k) {
    ...
    cs225::HSLAPixel &exit =
        solvedMaze->getPixel((endX * 10) + k, (endY + 1) * 10);
    exit.h = 0.0;
    exit.s = 0.0;
    exit.l = 1.0;
    exit.a = 1.0;
