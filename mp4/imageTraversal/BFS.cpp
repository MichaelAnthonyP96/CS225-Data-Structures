...
BFS::BFS(const PNG &png, const Point &start, double tolerance)
    : image(png), start(start), tolerance(tolerance) {
  /** @todo [Part 1] */
...
// IF THE ENTIRE DFS OBJECT IS ON THE HEAP DO I NEED TO IMPLEMENT THE 2D ARRAY
// ON THE HEAP AS WELL?
BFS::~BFS() {
  // for(unsigned i= 0; i < image.width(); ++i){
...
Point BFS::pop() {
  ...
  Point p = q.front();
  // while the queue is not empty and the front of the queue has been visited
  // before, pop off all elements until one hasnt been visited or the queue is
  // empty.
  ...
  // if the queue is empty return an arbitrary point, when the != operator is
  // invoked it will see that the queue is empty and terminate the process
  if (q.empty())
    return Point(0, 0);
