...
Point DFS::pop() {
  ...
  Point p = s.top();
  // while the stack is not empty and the top of the stack has been visited
  // before, pop off all elements until one hasnt been visited or the stack is
  // empty.
  ...
  // if the stack is empty return an arbitrary point, when the != operator is
  // invoked it will see that the stack is empty and terminate the process
  if (s.empty())
    return Point(0, 0);
