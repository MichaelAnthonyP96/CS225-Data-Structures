...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  //a for loop to iterator over all imageTraversals and Colorpickers, the index is the same for both
  for(unsigned i = 0; i < imagTrav_.size(); ++i){
    ...
    for(ImageTraversal::Iterator itI = imagTrav_[i]->begin(); itI != imagTrav_[i]->end(); ++itI){
      --frameCount;
