...
void SkipList::insert(int key, HSLAPixel value) {
    ...
    while (traverse->nodePointers[0].next != tail_ && (level >= 0)) {
        ...
        if (key < nextKey) {
            level--;
            if (level < 0 ) { break; }//possibly return?
        } else  {
    ...
    while (forwardLevel < maxLevel) {
        if (forward == tail_) {
            ...
            forwardLevel++;
        }else if (forward->nodePointers.size() > (size_t)forwardLevel) { //changed if to else if
            forward->nodePointers[forwardLevel].prev = newNode;
            newNode->nodePointers[forwardLevel].next = forward;
            forwardLevel++;
        } else {
    ...
    while (backwardLevel < maxLevel) {
        if(prev == head_) {
            ...
            backwardLevel++;
        }else if (prev->nodePointers.size() > (size_t)backwardLevel) { //changed if to else if
            prev->nodePointers[backwardLevel].next = newNode;
            newNode->nodePointers[backwardLevel].prev = prev;
            backwardLevel++;
        } else {
...
SkipNode * SkipList::findRHelper(int key, int level, SkipNode * curr) {
    ...
    SkipNode* ret = NULL; //initialized ret to NULL, trying to avoid garbage values
    ...
    // Recusive Case:
        ...
        ret = findRHelper(key, level-1, curr);//changed level to level-1 since nextKey > key
    } else {
