...
void SkipList::insert(int key, HSLAPixel value) {
  ...
  while (forwardLevel < maxLevel) {
    if (forward == tail_) {
      ...
      forwardLevel++;
    } else if (forward->nodePointers.size() >
               (size_t)forwardLevel) { // changed if to else if
      forward->nodePointers[forwardLevel].prev = newNode;
      newNode->nodePointers[forwardLevel].next = forward;
      forwardLevel++;
    } else {
  ...
  while (backwardLevel < maxLevel) {
    if (prev == head_) {
      ...
      backwardLevel++;
    } else if (prev->nodePointers.size() >
               (size_t)backwardLevel) { // changed if to else if
      prev->nodePointers[backwardLevel].next = newNode;
      newNode->nodePointers[backwardLevel].prev = prev;
      backwardLevel++;
    } else {
...
SkipNode *SkipList::findRHelper(int key, int level, SkipNode *curr) {
  ...
  SkipNode *ret =
      NULL; // initialized ret to NULL, trying to avoid garbage values
  ...
  // Recusive Case:
  else if (nextKey > key) { // changed if to else if?
    ret = findRHelper(key, level - 1,
                      curr); // changed level to level-1 since nextKey > key
  } else {
    ret = findRHelper(key, level, curr->nodePointers[level].next);
  }

  return ret; // changed NULL to ret
}
...
SkipNode *SkipList::findI(int key) {
  ...
  while (traverse->nodePointers[0].next != tail_ && level >= 0) {
    ...
    if (nextKey == key) {
      ...
      return retNode; // added this
    } else if (key < nextKey) {
      level--;
    } else {
      traverse = traverse->nodePointers[level].next;
      // level=traverse->nodePointers.size()-1; // slightly slows down code in
      // our sorted inserts
    }
  }
  return retNode; // changed NULL to retNode
}
...
bool SkipList::remove(int key) {
  ...
  if (node == NULL) {
    return false; // changed ret to return
  }
...
vector<int> SkipList::traverse() {
  ...
  while (listPrintingTraverser != NULL) { // changed tail to NULL
    ret.push_back(listPrintingTraverser->key);
    listPrintingTraverser = listPrintingTraverser->nodePointers[0].next;
  }
  // ret.push_back(tail_->key); // added this
  // push back the sentinel tail key value?
  return ret;
}
